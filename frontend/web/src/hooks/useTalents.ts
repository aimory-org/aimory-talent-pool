...
import { listTalents, updateTalentStatus, type ListTalentsParams } from '@/lib/api'
...
export function useTalents(options: UseTalentsOptions = {}): UseTalentsResult {
  ...
  const updateStatus = useCallback(async (pk: string, status: CandidateStatus) => {
    // Optimistic update
    const previousTalents = talents
    setTalents(prev => 
      prev.map(t => t.pk === pk ? { ...t, status, updated_at: new Date().toISOString() } : t)
    )
    
    try {
      await updateTalentStatus(pk, status)
    } catch (err) {
      // Rollback on error
